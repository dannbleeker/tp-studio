...
- **Render-engine layout pass (top priority)** — *Session 136 first-pass landed: tightened spacing (rankSep 80 → 60, nodeSep 40 → 32 in `src/domain/constants.ts`) + new `Re-layout diagram` palette command + design doc at [docs/RENDER_ENGINE_NOTES.md](docs/RENDER_ENGINE_NOTES.md). What's still TBD per the doc: real obstacle-aware edge routing (recommend dagre + custom routing pass; ELK as the longer-term fallback). The "edges hidden behind nodes" item depends on this.*
