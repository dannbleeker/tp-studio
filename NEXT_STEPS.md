...
- **Tier-2 #13 — Mutation testing one-time pass.** ⚠️ **Infrastructure landed Session 115; first-run blocked on a dial-in problem.** `@stryker-mutator/core` + `@stryker-mutator/vitest-runner` + `@stryker-mutator/typescript-checker` installed; `stryker.config.mjs` configured to mutate `src/domain/**` with `vitest` as the test runner and HTML reporter. First-run hit "Vitest failed to find test files related to mutated files" — Stryker's vitest runner uses `vitest --related` for per-mutant test filtering, but our tests use `@/` path aliases which don't show up in vitest's source-relationship graph. Without `--related`, the full 1195-test suite runs per mutant × 6601 mutants = multi-day runtime, not viable. Three fix paths to try: (a) convert `tests/` imports from `@/foo` to relative paths, (b) configure vitest's `deps.inline` / similar so alias resolution is visible to related-test detection, (c) scope mutation runs to single modules with explicit test paths. Each needs ~30-60 min of testing. Total dial-in cost: 2-3 hours focused; the install + config are durable.
