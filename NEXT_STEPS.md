...
4. **Multi-document tabs** — *Batches 2.1–2.3 + 4.1–4.2 + 5.1 shipped Session 138; still single-tab on screen.* Full plan at [docs/MULTI_DOC_TABS_PLAN.md](docs/MULTI_DOC_TABS_PLAN.md) — 6 phases, all 7 design questions answered. Done: Phase 1 (invisible refactor), **2.1** (state shape), **2.2** (per-doc localStorage + tab manifest + dual-write boot/migration), **2.3** (per-doc history infra), **4.1 + 4.2** (cross-doc services + the full `currentDoc` read-seam migration — Phase 4 complete), **5.1** (the **tab engine**: `setActiveDoc` invariant flip + `openTab`/`switchTab`/`closeTab`/`reorderTabs`/`duplicateTab`, store-only, fully unit-tested — the data model now genuinely supports N tabs). Next: **Batch 5.2** — the visible `TabStrip` UI (chips between TopBar↔Canvas, `+`/X/drag) + Cmd+T/W/1–9 + palette commands; then **5.3** (route doc-loads to new tabs + Settings toggle) and **5.4** (boot N-tab restore). (Phase 3's leftover per-tab search reset folds into 5.2.)
...
- **Multi-document tabs** — *Batches 2.1–2.3 + 4.1–4.2 + 5.1 shipped Session 138; single-tab on screen.* See [docs/MULTI_DOC_TABS_PLAN.md](docs/MULTI_DOC_TABS_PLAN.md) for the 6-phase arc. The data model, per-doc persistence + manifest, per-doc history, cross-doc services, the full `currentDoc` seam, AND the **tab engine** (open/switch/close/reorder/duplicate, unit-tested) are all in place under the hood. Next: Batch 5.2 (the visible `TabStrip` UI) → 5.3 (load-routing + toggle) → 5.4 (boot N-tab restore), whenever Dann is ready.
