...
### 4. Component-level interaction tests — ✅ mostly done
...
- **Storybook for the UI primitives.** Parked. Current primitive count is ~6 (Button, Field, MarkdownField, MarkdownPreview, ConfirmDialog, ErrorBoundary); the dev-dep cost (~25-50 MB) and ongoing maintenance is ahead of the discoverability win. Existing component tests cover the functional surface. Trigger: revisit when the primitive count doubles or contributors arrive who'd benefit from a visual playground.
