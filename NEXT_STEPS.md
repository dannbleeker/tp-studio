...
- **Multi-document tabs** — *Batches 2.1–2.3 + 4.1–4.2 + 5.1 + 5.2-core shipped Session 138; tabs now visible.* See [docs/MULTI_DOC_TABS_PLAN.md](docs/MULTI_DOC_TABS_PLAN.md) for the 6-phase arc. The full data/persistence/history foundation, the `currentDoc` seam, the tab engine, AND the visible `TabStrip` (chips/switch/close/new) are all in. Next: 5.2b (keyboard + palette + drag-reorder) → 5.3 (load-routing + toggle) → 5.4 (boot N-tab restore). The 5.2 PR is gated on a real-browser visual review.
