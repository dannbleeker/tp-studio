...
- **Render-engine layout pass (top priority)** — *Session 136 first-pass landed: tightened spacing (rankSep 80 → 60, nodeSep 40 → 32 in `src/domain/constants.ts`) + new `Re-layout diagram` palette command + design doc at [docs/RENDER_ENGINE_NOTES.md](docs/RENDER_ENGINE_NOTES.md). Obstacle-aware routing planned across four phases at [docs/EDGE_ROUTING_PROPOSAL.md](docs/EDGE_ROUTING_PROPOSAL.md). **Phase A landed**: API contract + `routeEdge` returning bezier verbatim + `useEdgeRoutes` hook gated behind `SMART_ROUTING_ENABLED = false`. Phases B + C land behind the same gate; Phase C is the first user-visible release (combined A+B+C with the Settings → Display radio defaulting `'smart'`). The "edges hidden behind nodes" item below tracks the visible fix.*
- **AND connector rendering + drag-drop creation** — *current state confirmed Session 136: the junctor circle already renders below the target with edges converging into it + one short line out, per the spec's "edges feed into the AND box first" requirement. What's still TBD: the drag-edge-onto-junctor gesture (drag an edge body, drop on an existing AND circle → `addCoCauseToEdge`). Approach + recommendation captured in [docs/RENDER_ENGINE_NOTES.md](docs/RENDER_ENGINE_NOTES.md) §2.*
