...
4. **Multi-document tabs** — *Batches 2.1–2.3 + 4.1 shipped Session 138; still single-tab.* Full plan at [docs/MULTI_DOC_TABS_PLAN.md](docs/MULTI_DOC_TABS_PLAN.md) — 6 phases, all 7 design questions answered. Done: Phase 1 (invisible refactor — `currentDoc()` + dormant per-doc keys), **2.1** (state shape: `docs`/`activeDocId`/`tabOrder` via `activeDocState`), **2.2** (per-doc localStorage slots + tab manifest + dual-write boot/migration), **2.3** (per-doc history infra — `historyByDoc` + `applyTabSwitchHistory`), **4.1** (the three doc-scoped services — clipboard/nudge/canvasRef — on the `currentDoc` seam + nudge keys on `activeDocId`). Next: **Batch 4.2** — migrate the remaining ~191 component `state.doc` reads to `currentDoc(s)` (its own focused, high-volume mechanical session) — then **Phase 5** (the visible tab-strip UI + tab actions, where `activeDocState` becomes multi-tab-aware).
...
- **Multi-document tabs** — *Batches 2.1–2.3 + 4.1 shipped Session 138; still single-tab.* See [docs/MULTI_DOC_TABS_PLAN.md](docs/MULTI_DOC_TABS_PLAN.md) for the 6-phase arc. The data model (`docs`/`activeDocId`/`tabOrder`), per-doc persistence + tab manifest, per-doc history infra, and the three cross-doc-aware services are now in place under the hood. Next: Batch 4.2 (bulk `state.doc` → `currentDoc` component sweep) then Phase 5 (the visible tab strip), whenever Dann is ready.
