...
4. **Multi-document tabs** — *Phase 1 + Phase 2 (Batches 2.1 + 2.2) shipped Session 138; still single-tab.* Full plan at [docs/MULTI_DOC_TABS_PLAN.md](docs/MULTI_DOC_TABS_PLAN.md) — 6 phases, all 7 design questions answered. Done so far: Phase 1 (invisible refactor — `currentDoc()` + dormant per-doc keys), **Batch 2.1** (multi-doc state shape: `docs`/`activeDocId`/`tabOrder` kept in lockstep by `activeDocState`), **Batch 2.2** (per-doc localStorage slots + tab manifest + dual-write boot/migration). Next: Batch 2.3 (per-doc history, deferrable) then Phase 3+ (the actual tab-strip UI).
...
- **Multi-document tabs** — *Phase 1 + Phase 2 (Batches 2.1 + 2.2) shipped Session 138; still single-tab.* See [docs/MULTI_DOC_TABS_PLAN.md](docs/MULTI_DOC_TABS_PLAN.md) for the 6-phase arc. The data model (`docs`/`activeDocId`/`tabOrder`) + per-doc persistence + tab manifest are now in place under the hood; the tab-strip UI (Phase 3+) rides on top whenever Dann is ready.
