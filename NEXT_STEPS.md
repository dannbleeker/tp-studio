...
4. **Multi-document tabs** — *Batches 2.1–2.3 + 4.1–4.2 + 5.1 + 5.2-core shipped Session 138; tabs are now VISIBLE.* Full plan at [docs/MULTI_DOC_TABS_PLAN.md](docs/MULTI_DOC_TABS_PLAN.md) — 6 phases, all 7 design questions answered. Done: Phase 1 (invisible refactor), **2.1–2.3** (state shape · per-doc persistence + manifest · per-doc history), **4.1–4.2** (cross-doc services + full `currentDoc` seam), **5.1** (the tab engine — `setActiveDoc` flip + open/switch/close/reorder/duplicate), **5.2-core** (the visible `TabStrip` — chip bar, click-switch, close-X, `+` new tab), **5.4** (boot restores ALL tabs on reload + force-persist the outgoing tab). PR #23 (gated on visual review) covers 5.2-core + 5.4. Next: **5.2b** (Cmd+T/W/1–9 + tab palette commands + drag-to-reorder), **5.3** (route doc-loads to new tabs + Settings toggle). Phase 3's leftover per-tab search reset folds into 5.2b.
...
- **Multi-document tabs** — *Batches 2.1–2.3 + 4.1–4.2 + 5.1 + 5.2-core shipped Session 138; tabs now visible.* See [docs/MULTI_DOC_TABS_PLAN.md](docs/MULTI_DOC_TABS_PLAN.md) for the 6-phase arc. The full data/persistence/history foundation, the `currentDoc` seam, the tab engine, the visible `TabStrip` (chips/switch/close/new), AND boot-restore (reload brings back all tabs) are all in. Next: 5.2b (keyboard + palette + drag-reorder) → 5.3 (load-routing + toggle). The 5.2 PR (#23, incl. 5.4) is gated on a real-browser visual review.
