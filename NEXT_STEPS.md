...
- ~~**Direction arrows on causal connectors.**~~ ✅ *Session 171* — every causal /
  necessity connector now carries a clear cause→effect arrowhead. The root cause of
  the earlier "no arrows" state: `useGraphEdgeEmission` *did* set `markerEnd:
  ArrowClosed`, but React Flow's built-in marker scales with the thin ~1.5 px stroke
  (tiny) and can't be offset (no `refX`), so the tip landed on the target handle and
  hid under the handle dot. Replaced with custom SVG markers (`EdgeArrowMarkers`):
  fixed-size triangle (`userSpaceOnUse`), `orient="auto"`, and a `refX` past the tip
  so the arrowhead is pulled back off the handle dot. Colour reads the live palette
  (recolours for colourblind-safe / mono in place). **Shipped default-on, no toggle**
  (the sub-question's lean). The three deliberate exceptions are preserved: junctor
  groups share one output arrow (no per-cause arrows), mutex (EC D↔D′) stays
  symmetric/arrow-less, note edges stay dotted/arrow-less. Details in CHANGELOG.
