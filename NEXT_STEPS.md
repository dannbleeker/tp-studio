...
4. **Multi-document tabs** — *Phase 1 + Phase 2 + Batch 2.3 (Batches 2.1–2.3) shipped Session 138; still single-tab.* Full plan at [docs/MULTI_DOC_TABS_PLAN.md](docs/MULTI_DOC_TABS_PLAN.md) — 6 phases, all 7 design questions answered. Done so far: Phase 1 (invisible refactor — `currentDoc()` + dormant per-doc keys), **Batch 2.1** (multi-doc state shape: `docs`/`activeDocId`/`tabOrder` kept in lockstep by `activeDocState`), **Batch 2.2** (per-doc localStorage slots + tab manifest + dual-write boot/migration), **Batch 2.3** (per-doc history infra — `historyByDoc` + the pure `applyTabSwitchHistory`, the `switchTab` prerequisite). Next: Phase 4 (cross-doc-aware services + migrate remaining `s.doc` reads to `currentDoc`) then Phase 5 (the visible tab-strip UI + tab actions, where `activeDocState` becomes multi-tab-aware).
...
- **Multi-document tabs** — *Phase 1 + Phase 2 + Batch 2.3 (Batches 2.1–2.3) shipped Session 138; still single-tab.* See [docs/MULTI_DOC_TABS_PLAN.md](docs/MULTI_DOC_TABS_PLAN.md) for the 6-phase arc. The data model (`docs`/`activeDocId`/`tabOrder`), per-doc persistence + tab manifest, and per-doc history infra (`historyByDoc` + `applyTabSwitchHistory`) are now in place under the hood; Phase 4 (service awareness) then Phase 5 (the visible tab strip) ride on top whenever Dann is ready.
