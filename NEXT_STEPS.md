...
- ~~**Selection visibility on the canvas — make it more obvious.**~~ ✅ **Done (Session 133, commit `053a41c`).** Shipped (a)+(d) per the triage recommendation. `.react-flow__node.selected` now carries a `filter: drop-shadow(0 0 10px rgb(99 102 241 / 0.55))` halo (dark-mode swap to indigo-400 at 0.65). New `useSelectionDimming` hook mirrors `useSearchDimming`'s shape exactly: single-item selection (entity / edge / group) attaches a `tp-focus-dim` className to non-focus nodes/edges at 0.5 opacity (milder than search's 0.18 — selection is more frequent so a deep dim would feel aggressive). Focus set keeps the selected item's 1-hop neighbourhood bright. Multi-select skipped (the selection itself is already the signal). Search-dim composes cleanly: search wins visually when both apply.
