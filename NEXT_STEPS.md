...
- **UI review by expert agent** — *audit done Session 135; batch 1 of 3 actioned.* A code-level design pass produced [docs/DESIGN_AUDIT_SESSION_135.md](DESIGN_AUDIT_SESSION_135.md) — 25 ranked findings, 3 suggested batches. **Batch 1 done** (findings 1–3): `<Field>` now emits real label/fieldset semantics (closed the accessible-name gap across the inspector), a shared `EYEBROW` token replaced the 3-size drift, and the inspector header reads as an `<h2>` heading. **Remaining: batch 2** (findings 4, 17, 11 — `TOGGLE_BUTTON_BASE` constant + `<ButtonGroup>` extraction + `<TabBar>` extraction) and **batch 3** (findings 10, 16 — `<InsetCard tone>` + frosted-glass opacity unification), plus the incremental lower-impact items (5, 6, 8, 18, 19, 20, 21…).
