...
- **Selection visibility on the canvas — HALF done (Session 133).** Shipped option (a): a soft drop-shadow halo on `.react-flow__node.selected` (CSS commit `1852acb`). Option (d) — selection-anchored dimming of non-focus nodes/edges — was attempted in commit `053a41c` but **reverted** because the `useSelectionDimming` hook caused a React Flow reconciliation infinite loop in the production build (React error #185 "Maximum update depth exceeded", confirmed via Playwright trace). The map+spread pattern produced new node object references on every render, which composed with React Flow 12.x's internal state syncing in a way that triggered runaway updates and the canvas crashed into its ErrorBoundary fallback. `useSearchDimming` uses the same shape but doesn't expose the bug because search is rarely active in tests. If reopened, the dim half needs a different architecture — candidates: CSS-only via a `data-focus-anchor` attribute on the canvas wrapper plus a `:has` selector; or driving the dim through React Flow's per-node `style` prop instead of cloning node objects; or a CSS custom property the selected node sets and siblings read via cascade.
