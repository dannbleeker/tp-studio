...
4. **Multi-document tabs** — *Batches 2.1–2.3 + 4.1–4.2 shipped Session 138; Phase 4 complete; still single-tab.* Full plan at [docs/MULTI_DOC_TABS_PLAN.md](docs/MULTI_DOC_TABS_PLAN.md) — 6 phases, all 7 design questions answered. Done: Phase 1 (invisible refactor), **2.1** (state shape via `activeDocState`), **2.2** (per-doc localStorage + tab manifest + dual-write boot/migration), **2.3** (per-doc history infra — `historyByDoc` + `applyTabSwitchHistory`), **4.1** (the three doc-scoped services on the `currentDoc` seam + nudge keys on `activeDocId`), **4.2** (the bulk sweep — all 179 store-derived `state.doc` reads across 61 files now go through `currentDoc`, so `doc` is read in exactly one place outside the store). Next: **Phase 5** — the visible tab-strip UI + `openTab`/`switchTab`/`closeTab`/`reorderTabs` + Cmd+T/W/1–9, where `activeDocState` finally becomes multi-tab-aware. (Phase 3's remaining bits — per-tab search reset, speculation-drop-on-switch — fold into Phase 5.)
...
- **Multi-document tabs** — *Batches 2.1–2.3 + 4.1–4.2 shipped Session 138; Phase 4 complete; still single-tab.* See [docs/MULTI_DOC_TABS_PLAN.md](docs/MULTI_DOC_TABS_PLAN.md) for the 6-phase arc. The data model, per-doc persistence + tab manifest, per-doc history infra, the cross-doc-aware services, AND the full `currentDoc` read-seam migration (all 179 reads) are now in place under the hood. Next: Phase 5 (the visible tab strip + tab actions), whenever Dann is ready.
