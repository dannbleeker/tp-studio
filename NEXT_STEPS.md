...
- **UI review by expert agent** — *audit done Session 135; batches 1 & 2 of 3 actioned.* A code-level design pass produced [docs/DESIGN_AUDIT_SESSION_135.md](DESIGN_AUDIT_SESSION_135.md) — 25 ranked findings, 3 suggested batches. **Batch 1 done** (findings 1–3): `<Field>` label/fieldset semantics, shared `EYEBROW` token, inspector `<h2>` header. **Batch 2 done** (findings 4, 5, 17, 11): `TOGGLE_BUTTON_BASE` constant, `<ButtonGroup>` primitive (Type + TitleSize converted), `<TabBar>` primitive (Inspector + Settings), mismatched selected/unselected colour pairs fixed. **Remaining: batch 3** (findings 10, 16 — `<InsetCard tone>` + frosted-glass opacity unification), plus the incremental lower-impact items (6 inline-action Button, 8 group-id UUID display, 18 focus-vs-focus-visible, 19 showModal, 20/21 AttributesSection primitives, 22–25 quick wins).
