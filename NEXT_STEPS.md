...
- ~~**No first-class Negative Branch Reservation diagram type**~~ ✅ **Done (Session 134).** New `'nbr'` diagram type registered across all per-diagram registries (palette, label, factory initial doc, layout strategy, handle orientation, method checklist, validator set, example, pattern library, default-entity-type). Palette: `injection / effect / ude / desiredEffect / assumption / note`. Validator set mirrors FRT (structural + cause-sufficiency + additional-cause on UDE + predicted-effect-existence). Diagram-type picker carries an NBR card; the pattern library has the QA-gate NBR starter. 7-step method checklist guides the canonical NBR walk (injection → forward to desired effects → turning point → UDEs → mitigation choice → CLR → decision). Risk-register CSV export (new `src/services/exporters/riskRegister.ts`, columns: risk_id / risk / trigger / consequence / mitigation / owner / status) gated by `requiresEntityType: 'ude'` so it only surfaces in the Export… picker when the doc has UDEs (works on NBR + CRT + any doc with UDEs). The reactive-vs-proactive mitigation distinction remains a medium-gap item (see below) — current implementation uses entity `attributes.owner` for ownership and infers mitigation status from whether an injection reaches the UDE; formal distinction tabled for a follow-up.
...
- ~~**Risk register export**~~ ✅ **Done (Session 134).** Bundled with the NBR diagram (major gap #5) — see above.

### Minor gaps (🟢)

- ~~**Pattern library / benchmarking / portfolio view**~~ — V2 spec items. **Sub-item A (reusable domain templates) ✅ Done (Session 134).** New "Pattern library…" palette command opens a picker (`PatternLibraryDialog`) listing every curated starter diagram in `src/domain/patterns/`. Each pattern carries a stable `id`, diagram-type-tagged label, one-line hint, and a builder function. The registry ships 9 starter patterns spanning CRT (2: customer-satisfaction, engineering-velocity), EC (2: work-life-balance, quality-vs-speed), and 1 each for FRT / PRT / TT / Goal Tree / S&T. Existing `EXAMPLE_BY_DIAGRAM` registry kept untouched (drives the quick `Load example…` path); pattern library is purely additive. Filter chip row at the top of the dialog lets users narrow to one diagram type. Click → load with an Undo toast. 9 new tests in `tests/domain/patterns.test.ts` guard the registry shape: every type has ≥1 pattern, ids are unique, builders don't throw, declared diagram-type matches built doc, schemaVersion is current. Growing the library: drop a new `<type>-<slug>.ts` in `src/domain/patterns/`, register in `PATTERNS` — that's it. Sub-items B (benchmarking / pattern recognition) and C (portfolio of improvement initiatives across multiple docs) remain parked — B needs AI integration (major gap #1) for embedding-based similarity; C needs cross-diagram traceability (major gap #3) + entity ownership (major gap #6).
...
- ~~**Move the React Flow Controls / minimap elsewhere on the canvas.**~~ ✅ **Done (Session 133, commit pending).** Shipped option (b) from the triage. New `CanvasNav` overlay merges the old `ZoomPercent` chip with React Flow's built-in `<Controls>` (zoom out / zoom in / fit-view) into one centred chip at bottom-centre. Uses `useReactFlow()` to dispatch the same imperative actions React Flow's Controls did, so behaviour is identical. MiniMap stays bottom-left as the only chrome in that corner now. The `data-component="zoom-percent"` selector was renamed to `"canvas-nav"`; print CSS + the overlay smoke test were updated accordingly. The deprecated `ZoomPercent.tsx` component was deleted.

- ~~**CLR map for the practitioner book.**~~ ✅ **Done (Session 134).** Two-pass: first landed as a raster slide extracted from Dann's `Theory of Constraints.pptx` (slide 2 — "Categories of Legitimate Reservations"); then rebuilt natively in HTML+SVG after Dann asked for searchable text, scalable vectors, and TPNode-style entity boxes. Final shape: `scripts/lib/clrMapHtml.mjs` is the single source of truth — exports `clrMapHtml()` (the `<div class="clr-map">…` block) and `CLR_MAP_CSS` (folded into the book's global stylesheet). Chapter 13 (`13-the-clr.md`) drops a `<!-- CLR_MAP -->` placeholder which `build-book-pdf.mjs` substitutes during the chapter-to-HTML pass. The 8 cards render in a 2×4 grid at the book's content width; each card has a header, bullet questions, and a TPNode-styled SVG vignette using the exact stripe palette from `src/domain/tokens.ts` (amber = rootCause / Cause, neutral = effect / Effect, red = ude / Missing, indigo = desiredEffect / New). The raster fallback (`docs/guide/diagrams/clr-map.png`) and the one-off PowerPoint COM extractor (`scripts/extract-clr-slide.py`) were removed once the native version shipped. `scripts/render-clr-map-native.mjs` remains as a preview renderer for inspecting CSS changes without rebuilding the full book PDF. PDF rebuilt; size went from ~1.0 MB to ~1.86 MB on the diagram's base64 expansion.

- ~~**Selection visibility on the canvas.**~~ ✅ **Done (Session 133 halo + Session 134 dimming).** Option (a) — soft drop-shadow halo on `.react-flow__node.selected` — shipped Session 133 (commit `1852acb`). Option (d) — selection-anchored dimming of non-focus nodes — landed Session 134 via a CSS-only architecture: the `:has(.react-flow__node.selected) .react-flow__node:not(.selected) { opacity: 0.5 }` rule on `.react-flow__viewport` dims everything-not-selected without any JS plumbing. The earlier hook-based attempt (`053a41c`) was reverted because the map+spread node-mutation pattern triggered React Flow 12.x reconciliation runaway updates (React error #185, confirmed via Playwright trace). The CSS approach is declarative, has zero per-render cost, and composes correctly with search-dim (search's deeper 0.18 wins on non-matches). Edges intentionally aren't dimmed — selecting a node doesn't mark its edges as `.selected`, so a node-driven edge-dim rule would hurt more than it helps; the halo already gives the focus signal.

- ~~**AND-junction creation via drag gesture.**~~ ✅ **Done (Session 133, commit pending).** Shipped a click-click substitute for a true drag-edge-onto-edge gesture (React Flow doesn't expose edge midpoints as drag handles, so a faithful drag would require a custom pointer pipeline). The new path: select a single edge → selection-toolbar verb **AND-join with another edge…** → click the second edge on the canvas → both edges get AND-grouped via the existing `groupAsAnd` action. While the mode is active, a StatusStrip chip "Click another edge to AND-join" surfaces; Esc / pane-click / clicking the source edge again all cancel. Palette command `start-edge-join-and` exists for keyboard parity. New state: `joinModeEdgeId` on `selectionSlice` + `startEdgeJoinMode` / `cancelEdgeJoinMode` actions. Esc cascade updated to consume join-mode before hoist + deselect.
...
- ~~**Update the book now that versions have changed and screenshots will have changed a bit.**~~ ✅ **Done (Session 132).** Triggered the `Update visual snapshots` workflow (PR #7) — 3 chapter screenshots refreshed (`chapter02-connected-pair`, `chapter03-causality-because`, `chapter14-revision-panel-open`). Foreword's "last reviewed" pin bumped from schema v7 / Session 103 → schema v8 / Session 132; Chapter 3's "v7 schema" version qualifier dropped (the sufficiency-vs-necessity edge.kind point is durable); AUTHORING.md's `currently v7` note bumped to v8. Also surfaced + fixed a long-standing cover-rendering bug in `scripts/build-book-pdf.mjs`: `.cover { height: 100vh }` was being evaluated against Playwright's layout viewport rather than the printed A4 page, clipping the cover-title off page 1; replaced with padding-based positioning that reliably places the cover content on page 1 (verified by decoding the PDF's compressed content streams + counting drawn glyphs per page — old build: ~20 glyphs on page 1; new build: 194 glyphs).
...
- ~~**Tier-2 #6 — `exactOptionalPropertyTypes` flag flip.**~~ ✅ **Done (Session 117).** Sessions 112 + 115 both evaluated and reverted. Session 117 finally landed it via a new `Patch<T>` mapped-type helper applied to `updateEntity` / `updateEdge` / `entityPatch` / `edgePatch` action signatures + ~12 construction-site fixes (conditional spread or destructured-rest "emit-or-omit") + 2 `as Entity` / `as Edge` casts at the merge sites. 0 tsc errors with the flag on; 1195 tests still passing.
- ~~**Tier-3 #14 — Lazy-load `MarkdownPreview` + DOMPurify + micromark.**~~ ✅ **Done (Session 115).** `MarkdownField` now `lazy()`-imports `MarkdownPreview` with a `<Suspense fallback="Loading preview…">` boundary. The component only renders when the user clicks the Preview tab (default is Edit), so the markdown deps only land for users who actively preview. **Index chunk: 118.56 KB → 86.74 KB gzip — a 32 KB / 27% reduction.** Two new lazy chunks: `MarkdownPreview-*.js` (22 KB gz, micromark + component) and `purify.es-*.js` (9 KB gz, DOMPurify). Bundle budget re-pinned in `bundle-budget.json` (index: 124 KB → 92 KB ceiling) so future regression catches earlier.
- ~~**Tier-2 #4 — Split `CreationWizardPanel.tsx` (596 LOC).**~~ ✅ **Partial (Session 115).** Step definitions (`GOAL_TREE_STEPS`, `EC_STEP_BY_SLOT`, `EC_STEPS`, `EC_STEPS_D_FIRST`) extracted into `src/components/canvas/creationWizardSteps.ts`. Panel file down 596 → 547 LOC. The panel's drag-handler + commit logic stays single-file — they share state in ways that resist further extraction without inventing a context that hurts more than it helps.
- ~~**Tier-2 #5 — `SettingsDialog` per-tab extraction.**~~ ✅ **Done (Session 121).** Each tab pulled into its own file under `src/components/settings/tabs/` (Appearance / Behavior / Display / Layout). Parent dropped 540 LOC → 89 LOC (modal shell + tab bar + active-tab switch). Each tab owns its own `useShallow` subscription so they don't re-render when sibling tabs' state changes. Existing 15 `SettingsDialog.test.tsx` tests pass unchanged.
...
- ~~**Tier-2 #13 — Mutation testing one-time pass.**~~ ✅ **Done (Session 121).** Dial-in fix: `vitest.related = false` (Stryker's own recommended fallback when `--related` can't trace through `@/` aliases). Per-mutant wall time ~7 s steady-state. Per-file recipe: `pnpm mutation --mutate src/domain/<file>.ts` completes in 5–20 min. First-pass score: **88.24%** on `paletteScore.ts` (30 / 34 mutants killed). Config header in `stryker.config.mjs` documents the blocker, the fix, the recipe, and a first-pass score table for trend tracking.
...
- ~~**Tier-3 #28 — Full hands-on keyboard navigation pass.**~~ ✅ **Automated portion done (Session 121).** Re-enabled the dialog axe scans dropped in Session 116 by adding `openHelp` / `openAbout` / `openSettings` to `window.__TP_TEST__` (deterministic store-action dispatch — no gesture flakiness). New tests: 3 dialog axe scans + 3 dialog Esc-close pins. Found-and-parked: `Modal` primitive renders `<dialog open>` without `.showModal()` and doesn't wire `useFocusTrap` — Tabbing past the last focusable element escapes the dialog. Captured below as its own follow-up. **The fully-manual hands-on portion** (workflow continuity, focus-order coherence as I Tab through, discoverability without a mouse) remains worth a periodic ~1-hour walkthrough by the author. Suggested manual checklist:
...
- ~~**Biome 2 strict-lint cleanup pass.**~~ ✅ **Done (Session 124, commit `01c9396`).** The 1.9 → 2.4 upgrade (Session 122) surfaced 24 findings; Session 124 cleared them all (`biome lint src tests` reports 0 warnings / 0 errors). Fix breakdown: 14 `useAriaPropsSupportedByRole` (added `role="img"` on decorative badge / indicator spans in TPNode + TPEdge, plus `<fieldset>` + `aria-label` on button-group containers in AppearanceTab + CreationWizardPanel); 4 `noStaticElementInteractions` (biome-ignore with rationale on React Flow node wrappers that own keyboard nav, plus MarkdownPreview's sanitized-HTML prose container that forwards onClick to embedded `<a>` anchors); 3 `noArrayIndexKey` (stable ids derived from wizard step content); 1 `useIterableCallbackReturn` (`.forEach` converted to no-return).
...
- ~~**Tier-3 #20 — IndexedDB migration.**~~ ❌ **Closed (Session 129)** — won't build without new evidence. Original gating was "do #19 first; migrate only if quota problems are real." With #19's auto-trim mitigation in place (Session 129), the quota path now self-heals: revisions get pruned when storage fills, the in-memory doc keeps editing, the user sees an actionable toast. The dominant reason for an IndexedDB rewrite (escaping the localStorage ceiling) is no longer pressure-tested. Migrating anyway would mean rewriting the entire sync persistence seam as async, touching ~30+ files including the recovery + autosave paths that are TP Studio's whole crash-safety story — multi-day with real regression risk. Re-open only if (a) a user reports quota-after-trim, or (b) we need a feature that depends on IndexedDB-only capabilities (cross-tab broadcast, blob storage, larger-than-quota docs). Until then, the localStorage path stays canonical.
...
> **Status (Session 95): ✅ Shipped.** Phase 1 (selection-verb registry + canvas helpers) landed in `adc95b9`; Phase 2 (SelectionToolbar component + unit + Playwright e2e) landed across `3fb91a6` … `df071f9`. Default ON; opt-out in Settings → Behavior. See CHANGELOG Session 95.
>
> **Verb-scope follow-ups (Sessions 96–97 + 127 + 128):** Mark as UDE / root cause (96), add-NC-child / promote-to-goal (96), add-prerequisite-need (97), cycle-edge-polarity (97), cycle-group-color (97), Spawn EC from selection / Start Negative Branch / Mark as CSF (127), Mark as Action / Outcome / Add precondition (TT, 128), Mark as Obstacle / IO / Add IO for obstacle (PRT, 128). The registry now covers the main per-diagram practitioner verbs across **CRT / FRT / Goal Tree / EC / TT / PRT** — all six TOC tree types.
