...
- **Render-engine layout pass (top priority)** — *Session 136 first-pass landed: tightened spacing (rankSep 80 → 60, nodeSep 40 → 32 in `src/domain/constants.ts`) + new `Re-layout diagram` palette command + design doc at [docs/RENDER_ENGINE_NOTES.md](docs/RENDER_ENGINE_NOTES.md). Obstacle-aware routing planned across four phases at [docs/EDGE_ROUTING_PROPOSAL.md](docs/EDGE_ROUTING_PROPOSAL.md). **Phases A + B + C landed**: edges now route around non-endpoint node bodies via a visibility-graph + A\* router (`src/domain/edgeRouting.ts`); user-visible toggle in Settings → Display ("Smart" / "Direct"), default `'smart'`. Phase D follows: junctor segment integration + per-layout route cache + USER_GUIDE / CHANGELOG polish.*
