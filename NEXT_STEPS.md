...
Pruned in Sessions 176, 193, 206, 209, and 211. The Session-206 pass removed the last of the shipped narrative after
...
The Session-211 pass walked every section again and cut nothing as shipped — the open items really are
...
~95% lines / ~84% branches. Exact current figures live in `public/stats.json` (regenerated by the `Stats`
workflow) — don't re-copy them here; this line said ~97/~85 until Session 211 precisely because someone did.
CI floors are `lines: 94` / `branches: 82` in `vite.config.ts`, auto-ratcheted by
`node ./scripts/pin-coverage-thresholds.mjs`
...
`CLAUDE.md` carries the environment guidance, now split by environment (Windows workstation vs Claude
Code on the web). **This section used to duplicate it and had gone stale in a way that mattered:** it
still recorded the Session-175 findings that `biome.exe` and the bundled Playwright Chromium were
AppLocker-blocked, both of which were superseded — the shim block is gone at the allow-listed
`C:\devtools` (Session 181) and Playwright runs locally (Session 169). Two memory files disagreeing about
whether a tool works is worse than one file saying it, so the copy is gone. Session 211.

The quirks that only lived here, kept because `CLAUDE.md` doesn't carry them:

- **No `jq` on the Windows box** — use `gh`'s built-in `--jq`, and query runs by the full 40-char SHA (a
  short SHA returns nothing).
- **`pnpm dlx` is blocked** there; `pnpm install` from `package.json` works.
- **`pnpm-workspace.yaml`** is sometimes autogenerated with anomalous content by pnpm; if `pnpm add`
  silently fails to update `package.json`, delete that file.
- **OneDrive sync + `node_modules`** is slow and lock-prone — the reason the project lives at
  `C:\devtools\tp-studio`.

### Lazy-loaded chunks
Pay their cost on demand: `html-to-image`, `dagre`+`@/domain/layout` (guarded by
`tests/build/dagreLazyLoadBoundary.test.ts`), `jspdf`+`svg2pdf.js`+`html2canvas`, `pptxgenjs`,
`PrintAppendix`, `CommandPalette`, `MarkdownPreview`+DOMPurify.
