...
- **Tier-2 #6 — `exactOptionalPropertyTypes` flag flip.** Sessions 112 + 115 both evaluated and reverted. Same 272 errors across 21 files. The pattern is dominated by `updateEntity` / `updateEdge` / `setDocument` action signatures using `Partial<Omit<T, ...>>` which under exactOptional doesn't accept `{ field: undefined }` (the clear-the-field idiom). Realistic fix: introduce a `PartialWithUndefined<T>` mapped type or `Patch<T>` helper, retype the store actions, then cascade fixes through call sites. **Estimated cost: 2-3 hours of focused refactor.** Worth doing as its own session — splitting it across a maintainability pass leaves the codebase in a half-flipped state, which is worse than not having the flag at all.
