...
- **UI review by expert agent** — *audit done Session 135; all 3 batches actioned.* A code-level design pass produced [docs/DESIGN_AUDIT_SESSION_135.md](DESIGN_AUDIT_SESSION_135.md) — 25 ranked findings, 3 suggested batches, **all done**: batch 1 (1–3) Field label/fieldset semantics + `EYEBROW` token + inspector `<h2>` header; batch 2 (4, 5, 17, 11) `TOGGLE_BUTTON_BASE` + `<ButtonGroup>` + `<TabBar>` + colour-pair fixes; batch 3 (10, 16) `<InsetCard tone>` + frosted-glass opacity. **All findings actioned except #19.** The incremental sweep cleared 6, 8, 13, 14, 15, 18, 20, 21, 22, 23, 24, 25. **Only remaining: #19** — migrate `LargeDialog` from `<dialog open>` + manual focus trap to native `showModal()`. *Deliberately deferred* — jsdom's `showModal`/`::backdrop` support is fragile and the change risks the dialog test suite for a lower-impact gain (a working focus trap + `aria-modal` is already in place). Revisit if a real-AT pass flags the page-behind-in-tab-order issue, or do the lower-risk `inert`-on-page-root variant the audit notes.
