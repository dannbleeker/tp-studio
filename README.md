...
**Live demo:** <https://tp-studio.struktureretsundfornuft.dk/> — installs as a desktop / mobile PWA and works fully offline after first visit: the app shell is precached, the export and preview vendor chunks plus the practitioner book warm into the cache on idle, and the app asks the browser for persistent storage so neither the cache nor your saved diagrams are evicted. **About → offline readiness** reports the state (worker, precache entries, persisted, cached size). Chrome and Edge will offer an Install prompt after a couple of visits; or open the command palette (`Ctrl/Cmd+K`) and pick **Install TP Studio…** to install on demand.
