...
**Live demo:** <https://tp-studio.struktureretsundfornuft.dk/> — installs as a desktop / mobile PWA and works fully offline after first visit: the app shell is precached, the export and preview vendor chunks plus the practitioner book warm into the cache on idle, and the app asks the browser for persistent storage so neither the cache nor your saved diagrams are evicted. **About → offline readiness** reports the state — service worker, precached files, whether storage is persisted, cache size, and whether the on-demand extras (export/preview chunks and the book) are cached — and lets you fetch whatever is missing on the spot, for when you have a short window of connectivity before going offline again. Chrome and Edge will offer an Install prompt after a couple of visits; or open the command palette (`Ctrl/Cmd+K`) and pick **Install TP Studio…** to install on demand.
...
**5,100+ tests** (5,032 unit + 96 Playwright e2e) across the suite ([tests/](tests/)), at ~95% line / ~84% branch coverage. Exact current figures are regenerated by the `Stats` workflow into [`public/stats.json`](public/stats.json) and rendered on the live dashboard — the numbers here are a deliberately rounded floor, not a maintained mirror. Coverage spans every layer:
...
The **change history is the source of truth** — see [CHANGELOG.md](CHANGELOG.md) (Sessions 150+) and [docs/CHANGELOG-archive.md](docs/CHANGELOG-archive.md) (Sessions 1–149); what's planned next is in [NEXT_STEPS.md](NEXT_STEPS.md).

**5,100+ tests green (≈95% line coverage), TypeScript + Biome clean, `pnpm audit --prod` clean.**
