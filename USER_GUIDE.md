...
**Save to file / Open from file (Chromium).** On Chrome / Edge, two extra palette commands let you work with a *real file on disk* instead of the browser's download / upload flow: `Cmd/Ctrl+K` → **Save to file…** writes the current document's `.tps.json` to a location you pick, and **Open from file…** reads one back into a new tab. These are purely additive — autosave to local storage, the tabs, and the Export/Import commands above all behave exactly as before; this just adds a file on disk as a target for the same JSON. The commands are hidden on browsers without the File System Access API (Firefox / Safari), where **Export as JSON** + **Import from JSON…** remain the path. **OneDrive tip:** save into your synced `OneDrive\…` folder and the OneDrive client backs the file up and syncs it across your devices with no account linking — **Open from file…** the same file on another machine to carry on.
