...
**Collapsible sections.** The inspector's secondary controls live in two **collapsible sections** — **Appearance** (title size + icon) and **Advanced** (unspecified-placeholder + locus) — both collapsed by default so the core Title / Type / Description / State fields stay above the fold. Click a section header to expand it; the open/closed state is remembered per section as you switch between entities and across reloads.
...
**Evidence.** Below the Owner block is an **Evidence** section — a structured list of citations, observations, measurements, stakeholder claims, and named assumptions backing the entity. Each row carries a description plus two compact colour-coded dropdowns you pick directly: a **source** (one of *Observed*, *Stakeholder*, *Metric*, *Policy*, *Assumption*) and a **strength** (*Weak*, *Moderate*, *Strong*). An optional URL field captures a citation link; the external-link icon next to it opens the URL in a new tab. A per-row **Mark validated** button stamps the current timestamp into the evidence item and credits the entity's Owner as the validator. Use the list to make provenance explicit at workshop time: "this UDE is a *strong/metric* — p95 = 740ms last week" reads very differently from "this UDE is a *weak/assumption*", and the difference often surfaces during the trim step of an FRT. Evidence items survive JSON export / share-link reload and feed the new `evidence` column of the risk-register CSV (rendered as `[strength/source] description (url)` entries, joined by semicolons).
...
**Verbalisation strip across the top.** When you open an EC document, a thin italic strip at the top of the canvas reads the cloud's verbal form continuously. Edit any of the 5 slot titles and the strip updates live; click an assumption-count chip in the strip to jump straight to that edge's Assumption Well. A small **D′-first** toggle re-reads the cloud leading with the D′ (own) side's need and tactic — Cohen's advice is to present to the listener's own side first so they feel heard; it defaults off (the canonical B-first order).
...
**Find (`Cmd/Ctrl+F`).** Opens a Find panel pinned near the top of the canvas. Type a query — it searches **entity titles + descriptions, group titles, edge labels, assumption text, and review-comment bodies** live (each result row shows what matched, with the entity's type for entity hits). Toggle case-sensitive, whole-word, or regex modes from the icons on the right; the regex toggle also accepts the `/pattern/flags` shorthand. Press `Enter` to jump to the next match, `Shift+Enter` for the previous. The canvas pans to center each match, and any collapsed groups in the way are automatically expanded; if you're hoisted somewhere else, the tool unhoists so the match is reachable. Jumping to an **assumption** selects its host edge (its assumptions show in the Edge Inspector); jumping to a **comment** selects the anchored object and opens the comments panel. While the panel is open and the query has at least one hit, non-matching nodes and edges fade to ~18% opacity so the matches stand out without losing the surrounding causal context.
...
**Reordering siblings in an auto-layout diagram.** Auto-layout decides the left-to-right order of nodes that share a row (siblings — e.g. the causes feeding one effect) by its own crossing-minimization heuristic. To override it, right-click a node → **Move earlier in layout** / **Move later in layout** to shuffle it among its row-mates. The order is remembered per entity and survives re-layouts (it's the same `ordering` that numbers Transition-Tree steps). The menu items appear only on auto-layout diagrams, and only when the node actually has a sibling to swap with; on hand-positioned diagrams (Evaporating Cloud) you just drag boxes instead.
...
**Statement-quality nudges.** A few reservations sharpen *how* an entity is written rather than how the tree is wired. A **single-word title** on a causal or necessity tree gets a gentle nudge — a cause or effect should say *what is happening* ("Backlog grows"), not just name a thing ("Backlog"); the terse-by-design diagrams (Evaporating Cloud, Strategy & Tactics, Freeform) are exempt, as are `unspecified` placeholders. Where **exactly two independent causes** feed one effect with no connector, the *additional-cause* reservation asks the magnitude question directly: is each cause enough on its own (leave them separate, or model as an OR) or only enough together (group them as an AND)? And on a **Goal Tree**, a Critical Success Factor whose title reads as *compliance* (law, regulation, GDPR, audit…) is nudged toward being a Necessary Condition a few layers down — a threshold you must not breach — rather than a make-or-break success driver.
...
**Jonah quick-check.** `Cmd/Ctrl+K` → **Jonah quick-check (fast 4-question pass)** opens a small stepper with four read-aloud questions covering the two axes a Jonah checks first — *entity* vs. *causality* × *existence* vs. *clarity*. Where per-edge scrutiny is exhaustive for one arrow and the walkthrough clears the warnings that fired, the quick-check is a fast triage over the *whole* diagram before you commit to either — "are the boxes clear and real, are the arrows real cause-and-effect, and is each cause enough on its own?" Tick each as you consider it (a session aid, nothing saved); it reads nothing and changes nothing, so it's available under Browse Lock too.
...
**The turning point.** The whole reservation hinges on *where* the injection's chain flips from helpful to harmful — the "yes, but…" moment. Mark it by setting that arrow's [polarity](#edge-polarity-positive--negative--zero) to **negative**. The canvas then draws the branch the way you read it: the **injection → UDE spine** stays at full strength while **side branches dim**, and the turning-point arrow carries a small rose **"turning point"** badge — so a busy NBR reads as one clear storyline at a glance. When you select an arrow, the Edge Inspector reinforces it: the turning-point arrow is called out in rose ("this is where the branch turns negative"), and every other NBR arrow carries a reminder to tag the point where things first go wrong. Each node already wears its role as its type label — Injection, Effect, Desired Effect, Undesirable Effect — so the branch reads top-to-bottom without extra chrome.
...
**Cloud type.** Below the verbal-style toggle, an EC document also has an optional **Cloud type** dropdown — *Dilemma, Conflict, UDE, Consolidated, Core,* or *Firefighting* — that labels the cloud's role in Cohen's *TP Basics* progression (UDE cloud → Consolidated → Core). Pick one and a small sky-blue chip appears next to the title; leave it "— Untyped" (the default) and nothing shows. The type never changes the diagram — a typed cloud reads, edits and breaks exactly like an untyped one — it drives guidance in two places. The **creation wizard** reads it to walk Cohen's per-type build order and show that type's guiding questions (see below). And once a type is set, a **"Best arrow to break"** note appears right under the dropdown with Cohen's recommendation for that kind of cloud — it stays there while you work the assumptions, which is when you need it, rather than vanishing with the wizard. Three ready-made clouds — **UDE cloud**, **Core cloud**, and **Firefighting cloud** — live in the Templates library (**Browse templates…**), each pre-tagged.
...
- The TT method checklist includes an **appropriate-condition** step (Session 198): for each action, test that you can actually take it (within your span of control, precondition true) *and* that it won't cause serious negative side-effects (spin off a Negative Branch if you're unsure). Record the *why* of each action in its **Need** field.
...
- **Ongoing objectives (Session 198).** An intermediate objective (or the goal) can be marked **Ongoing** — a checkbox in the inspector's Advanced section — when it's continuous work rather than a one-time deliverable. The node then shows an **"↻ ongoing"** chip so it doesn't read as a completable checkbox (Newbold's "What is Done?"). The flag also applies to a Goal-Tree goal.
...
- **Obstacle/objective intake table.** From a workshop obstacle brainstorm, `Cmd/Ctrl+K` → **Add obstacles + objectives (intake table)…** opens a table where each row pairs an Obstacle with the Intermediate Objective that overcomes it, plus an optional **show-stopper?** flag and a **blocking factor** note. **Apply** mints them all at once — one obstacle + one IO + the objective→obstacle link per row — as a single undo, so a whole list of blockers becomes a wired PRT skeleton in one step instead of one pair at a time. If the tree already has a single apex goal, each new obstacle is wired to it too. (The show-stopper flag is stored on the obstacle; the blocking-factor note becomes its description.) For a one-off pair, the older **Add Intermediate Objective for this Obstacle** verb still works from a selected obstacle.
...
- The palette types map onto the S&T facets:
  ...
  - **Necessary Condition** (lime) — an optional supporting-condition node. The three directional assumptions are **not** modelled as separate nodes: they're fields on the tactic's facet card (see below). This type is here for an alternate style if you'd rather model conditions as their own entities.
  - **Effect** (grey) + **Note** (yellow) round out the palette. (Assumptions aren't a palette node type — they're either the tactic's facet fields or annotations you add to an *edge* via the Assumption Well.)
- Build top-down with the facet cards: add the apex step as an `injection` and fill its **S&T facets** (Strategy, Parallel, and — once it has sub-steps — Sufficiency; the apex has no Necessary assumption). Add each sub-step as its own `injection` connected up to its parent, giving each a Necessary + Parallel assumption. Each step's tactic is the context its children's strategies serve.
- The Document Inspector's Method checklist carries seven S&T steps (analysis-first → apex strategy → tactic → NA → PA → SA → decompose) so you can tick off the discipline as you go.
- **First-class 5-facet card (directional).** Select any injection (tactic) on an S&T diagram; the inspector grows a new **S&T facets** section with four textareas — Necessary Assumption, Strategy, Parallel Assumption, Sufficiency Assumption — in that top-to-bottom order, with a directional read-aloud line built from what you've filled. Each assumption points a direction: **Necessary** justifies the step up to its parent, **Parallel** bridges the strategy and the tactic, **Sufficiency** justifies breaking the step down into sub-steps. Filling any one flips the canvas card into a tall 5-row layout beneath the tactic title, with plain directional labels (each carrying a hover tooltip that spells out its full meaning) — no more cryptic "NA / PA / SA". The Strategy row gets an indigo accent. Empty rows render as italic `(unset)` placeholders so the structural slot stays visible.
...
- **CLR rules.** Structural set plus three S&T-specific rules. **`st-tactic-assumptions`** (clarity tier) is position-aware: it asks a step for the necessary assumption only when it has a parent, the sufficiency assumption only when it has children, and the parallel assumption always — so the apex is never nagged for a necessary assumption, nor a leaf for a sufficiency one; the message names which required facets are still empty. **`st-tactic-rollup`** (sufficiency tier) fires on a *leaf* step — one that feeds a parent but has no children — because without children the structural sufficiency claim is empty; decompose it, or dismiss the warning if the leaf is genuinely atomic. **`st-tactic-fold-in`** (sufficiency tier) fires on a step that decomposes into exactly one sub-step — a real decomposition needs two or more jointly-sufficient children, or the single child should fold back in.
...
The library also carries a **Goldratt canon + published-case set** — twenty-one diagrams tracing the TOC literature itself. From *The Goal*: the failing-plant CRT, the constraint-turnaround FRT, the automation-efficiency NBR, and the *Make money now and in the future* (T/I/OE) Goal Tree — all labelled retrospective, because the Thinking Process tools were codified a decade after that book. From *It's Not Luck*, the novel that actually showcases the tools: the divest-or-grow conglomerate cloud, the commodity price trap, the un-refusable market offer with its NBR and rollout PRT, pull replenishment (retold retail-side in *Isn't It Obvious?*), and the parent-vs-teenager everyday cloud. From *Critical Chain*: *Why projects slip* and *Critical-chain buffering*. From *The Choice*: the forecast-committed fashion CRT and its forecast-vs-react cloud. And a six-diagram **public-policy suite** — CRT, EC, Goal Tree, NBR, FRT, and PRT on one real question (alcohol sales in supermarkets) — paraphrased from Mabin & Cavana's open-access case study in *System Dynamics Review* (2024). Book-derived diagrams are abstracted reconstructions in TP Studio's own wording: the novels are referenced, never quoted, no character names appear, and each pattern file's source note records its lineage.
...
The **`goalTree-nc-depth`** check normally holds Necessary Conditions to Dettmer's two layers — the right discipline when the Goal Tree is feeding a Current Reality Tree. If you're building a deeper *stand-alone* conflict-resolution tree, run `Cmd/Ctrl+K` → **Toggle conflict-resolution NC depth (Goal Tree)** to opt into a relaxed five-layer cap; the mode is stored on the document and off by default.
...
**When you're offline, the app says so.** A small amber **Offline — your work is saved locally** chip appears in the bottom-right corner whenever the browser loses its connection, and disappears when it comes back. It is there so a missing network never reads as a broken app: everything on the canvas keeps working, and every edit keeps saving.

**Why your offline cache can disappear — and what the app does about it.** Browsers treat site data as disposable: under storage pressure a browser evicts whole origins, least-recently-used first, and that takes the cached app *and* the documents in `localStorage` with it. The app now asks the browser for **persistent storage** at start-up, which exempts it from that eviction; an installed app is normally granted this without asking you. It also checks at every start-up that the cache is genuinely intact, and quietly re-downloads it once if it isn't — so a cache that was wiped while you were online repairs itself before the next time you need it.

**Checking whether offline actually works.** `Cmd/Ctrl+K` → **About TP Studio…** now reports it directly: the service-worker state, how many files are cached, whether storage is persisted, how much space the cache uses, and whether the on-demand extras — the export/preview chunks and the practitioner book — are cached. That last row has a control beside it that fetches whatever is missing immediately, rather than waiting for the app to get round to it on idle. That is the one to use when you have a short window of connectivity and want to be certain you are ready before it disappears. The row never guesses: it counts what is actually in the cache, and when it genuinely cannot tell — no service worker is serving yet, or the browser will not let the app read its cache — it says **Unknown** rather than reporting a number it has not verified. If you ever get the browser's "No internet access" page when opening the app offline, this panel is the place to look — an active worker with **0** cached files means the cache was cleared, and the app will rebuild it the next time you open it with a connection. If it instead shows no service worker at all on a work machine, your browser is most likely configured by IT to clear site data, which no setting inside the app can override.

**Exports and the book work offline too.** PDF, PNG and PowerPoint export, the Markdown preview, and the practitioner book are deliberately not part of the first-visit download — they would triple it. Instead the app fetches them quietly in the background once it is idle, so they are ready offline without making the first visit slow.
