# Changelog archive — Sessions 1–149

Reverse chronological, same format and same purpose as [CHANGELOG.md](../CHANGELOG.md): the home of
record for *why* something is the way it is.

Split out in Session 211. The combined file had reached 11,816 lines, and `CLAUDE.md` tells every new
session to read it from the top for rationale — an instruction that only works if the file opens on
history that is still load-bearing. **Nothing was edited or deleted in the move**; these entries are
byte-for-byte what they were.

Sessions 150 and later are in [CHANGELOG.md](../CHANGELOG.md).
