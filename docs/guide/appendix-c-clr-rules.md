...
| `goalTree-nc-depth` | clarity | An NC nested deeper than two layers below a CSF. Dettmer's checklist stops at two — deeper detail is execution planning, better developed in a Prerequisite Tree. Depth is the shallowest path when an NC supports several parents. The cap relaxes to **five** layers when the doc opts into *conflict-resolution mode* (palette → *Toggle conflict-resolution NC depth*) for a deeper stand-alone tree. |
