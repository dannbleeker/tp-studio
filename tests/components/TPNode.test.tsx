...
describe('TPNode — entity-type-specific rendering', () => {
  ...
  it('note entities render both React Flow handles with the same visible styling as causal entities (Session 136)', () => {
    // Session 136 — two-step evolution. First (batch 4): missing
    // handles dropped FL-imported note-edges, so we restored them as
    // hidden + non-connectable. Dann then asked to support new note-
    // edges in general, with a dotted render — so the handles now
    // match the causal-entity treatment (visible, connectable). The
    // visual differentiator moved off the handle and onto the edge:
    // note-touching edges render dotted + thinner in `TPEdge.tsx`.
    const entity = createEntity({ type: 'note', title: 'A note', annotationNumber: 1 });
    const { container } = mountWithRF(<TPNode {...makeNodeProps({ entity })} />);
